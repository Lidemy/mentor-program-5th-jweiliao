...
<body>
  ...
  <div class="container-wrapper">
    ...
    <?php
        $stmt = $conn->prepare(
          'SELECT count(id) AS count FROM wei_blog_article WHERE is_deleted IS NULL'
        );
        $result = $stmt->execute();
        if(!$result) {
          die("Error: " . $conn->error);
      ...
      <ul class="pagination">
        <?php if($page !== 1) { ?>
          <li><a href="article_list.php?page=1">第一頁</a></li>
          <li><a href="article_list.php?page=<?php echo $page - 1 ?>">上一頁</a></li>
        <?php } ?>
        <?php if($page !== $total_page) { ?>
          <li><a href="article_list.php?page=<?php echo $page + 1 ?>">下一頁</a></li>
          <li><a href="article_list.php?page=<?php echo $total_page ?>">最末頁</a></li>
        <?php } ?>
      </ul>
      <?php } ?>
    </div>
